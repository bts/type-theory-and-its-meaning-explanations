...
implementations of ytpe theory for proof assistants. In practice,
...
We introduce four syntactic categories (sorts):\\

\begin{tabular}{lll}
  \toprule
  sort & names & description\\ \midrule
  $\val$ & $M, N, A, B$ & values (canonical forms and types) \\
  $\utm$ & $U, V$ & uses (eliminators) \\
  $\rtm$ & $R, S$ & neutral terms (variables and applied uses) \\
  $\mtm$ & $E, F$ & expressions (normal forms, i.e.\ canonical forms, neutral terms)\\
  \bottomrule
\end{tabular}\\
...
like $\elim{\ufst}{R}$. Lastly, an \emph{expression} is a value or it
is a neutral term. In our presentation, the injections from, for
instance, values into expressions will be left tacit, since it will be
...
\begin{tabular}{ll}
  ...
  $\lfsequent\Gamma\valof{M}{A}$ & ``$M$ is a canonical verification of $A$ in context $\Gamma$''\\
  $\lfsequent\Gamma\useof{U}{A}{x}{B(x)}$ & ``$U$ is a use of $x:A$ yielding $B(x)$ in context $\Gamma$''\\
...
Note that we have made all the above judgements with respect to a
...
latter represents derivability, whereas the former encompasses
