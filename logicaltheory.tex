...
\framebox{$\isprop{P}$} and \framebox{$\istrue{P}$}; and what is $P$?
...
\framebox{$\hyp{\mathcal{J}}{\mathcal{J}'}$}.Its meaning explanation
is as follows:
...
\framebox{$\genj{x}{\mathcal{J}}$}. The meaning explanation for this
new judgement is as follows:

\begin{quote}
  To know the judgement $\genj{x}{\mathcal{J}}$ is, for any arbitrary
  expression $E$,   \footnote{Technically, $E$ is qualified as being of
the same valence as $x$, but because we have not developed a formal
theory of expressions in this presentation, I choose to ignore this
issue.}      to know $[E/x]\mathcal{J}$ (i.e. the substitution of $E$ for
  $x$ in the expression $\mathcal{J}$.
\end{quote}

As far as notation is concerned, the general judgement bar binds the
least tightly of all the other notations we have considered. Likewise,
